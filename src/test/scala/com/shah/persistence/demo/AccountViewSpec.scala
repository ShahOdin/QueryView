...
class AccountViewSpec extends TestKit(ActorSystem("test-system")) with ImplicitSender
  ...
  def getAccountReader(): ActorRef = {
    ...
    Mock.AccountView.startActor(system, 7)
  }

  def getAccountWriter(): ActorRef = {
    AccountApi.startActor(system)
  }
  ...
  "AccountView" should {
    ...
    "be tested with the correct memoryReadJournal" in {
      ...
      import akka.NotUsed
      def assertAcceptedTransitionsReceived(eventSource: Source[EventEnvelope, NotUsed],
      ...
      assertAcceptedTransitionsReceived(allEvents, 2)
